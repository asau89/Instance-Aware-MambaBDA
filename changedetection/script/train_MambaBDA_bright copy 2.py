...
class Trainer(object):
    def __init__(self, args):
        ...
        # Instantiate the instance consistency loss
        ...
        self.inst_losses = []
        self.final_losses = []
        self.inst_loc_losses_unweighted = [] # For unweighted instance localization loss
        self.inst_clf_losses_unweighted = [] # For unweighted instance classification loss
        ...
        for itera_idx in progress_bar:
            ...
            with autocast():
                ...
                labels_clf_down = F.interpolate(labels_clf.unsqueeze(1).float(),
                  ...
                  # --- Instance Consistency Losses ---
                loss_inst_loc = self.inst_consistency_loss(features_loc, labels_loc_down)
                loss_inst_clf = self.inst_consistency_loss(features_clf, labels_clf_down)
            ...
            self.inst_loc_losses_unweighted.append(loss_inst_loc.item())
            self.inst_clf_losses_unweighted.append(loss_inst_clf.item())


            # Update progress bar every iteration (set % 1 == 0 for every iteration print)
            if (itera + 1) % 1 == 0:
                progress_bar.set_postfix({
                    'BDA Loss': f'{bda_loss.item():.4f}',
                    'Inst Loss': f'{inst_loss.item():.4f}',
                    'Total Loss': f'{final_loss.item():.4f}',
                    'Inst Loc (Unw)': f'{loss_inst_loc.item():.4f}', # Unweighted
                    'Inst Clf (Unw)': f'{loss_inst_clf.item():.4f}'  # Unweighted
                })
...
def main():
    ...
    # Arguments for Instance Consistency Loss
    parser.add_argument('--alpha', type=float, default=0.5, help='Weight for the localization instance consistency loss.')
    parser.add_argument('--beta', type=float, default=0.5, help='Weight for the classification instance consistency loss.')
