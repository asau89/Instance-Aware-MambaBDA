    ...
    def training(self):
        ...
        for itera_idx in progress_bar:
           ...
           # ==============================
            ...
            # Optional: Gradient diagnostics (every 500 iters)
            if (itera + 1) % 500 == 0:
                with torch.no_grad():
                    total_grad_norm = 0.0
                    grad_count = 0
                    for name, p in self.deep_model.named_parameters():
                        if p.grad is not None:
                            gnorm = p.grad.norm().item()
                            total_grad_norm += gnorm
                            grad_count += 1
                    avg_grad_norm = total_grad_norm / max(grad_count, 1)
                    print(f"[Iter {itera+1}] Grad Norm={avg_grad_norm:.6f} | BDA={bda_loss.item():.4f} | Inst={inst_loss.item():.4f}")
            ...
            progress_bar.set_postfix({
                ...
                'Inst Loc (Unw)': f'{loss_inst_loc.item():.4f}',
                'Inst Clf (Unw)': f'{loss_inst_clf.item():.4f}'
            })
